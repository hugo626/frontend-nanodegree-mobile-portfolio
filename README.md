...
```
  gulp
```
This command will remove the content in ```dist/``` folder, then minify css,js,html files and copied into ```dist/``` folder with excatly structure. All images will be compressed, resized and copied into corresponsed folder.
...
```  
**Notice** Due to the GFW, the ```gulp pagespeed``` seems doesn't give the correct score when we use Ngrok.

## What Optimised
### Cam's Portfolio (index.html)
1. Images
    * download pictures for thumbnail, ```project-2048.jpg```,```project-mobile.jpg``` and ```project-webperf.jpg```.
2. Index.html
    ...
    * as the ```print.css``` will only be used when print the page, add media query.
      ...
      ```
    * As ```analytics.js``` only for analyse use, so we can loaded it asynchronous it. Also move ```google-font``` and ```analytics.js``` move to of ```<body>``` tag, so both of them would not block the browser to rendering webpage.
      ```html
      <body>
        ....
...
### Pizza Website
1. In ```main.js```
    * I removed function ```determineDx()``` as after I analysed, it is necessary and the size change is based on the percentage. So I moved the ```switch``` section to ```changePizzaSizes()``` function, and change the size of width to percentage 
      ```javascript
    ...
    * In ```updatePositions()``` function, ```document.body.scrollTop``` is called inside of for-loop, so it will make browser keep tring to Forced Synchronous Layout. The solution is, we extract the ```document.body.scrollTop``` out of the for lopp, and then cache it for using.
        ```javascript
...
2. Images
    ...
    After analyise, I observed both pictures are too large than their max size in web page, so I decide to resize both size to match with their max size in web page. For  example, in mobile view, the max size is 720 * 540 px. And in desktop view, the max size is only 360 * 270 px. In ```index.html``, it also try to refered ```pizzeria.jpg``` as a thumbnail picture (115 * 86 px). 
...
3. Pizza.html
    ...
    As I observed, when pizza.html is loaded in desktop view, the size of ```pizzeria.jpg``` is 
    360 * 270 px, but when browser's view port width is smaller than 992 px, the size of ```pizzeria.jpg``` will be resized to 720 * 540 px. So I decide to make this image in responive way.
